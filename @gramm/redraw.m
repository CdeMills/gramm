...
if (isempty (obj.redraw_cache)) % Takes a long time so we only do it once
  ...
  if (exist ('OCTAVE_VERSION', 'builtin'))
    set (obj.title_text_handle, 'units', 'normalized');
    set(obj.facet_text_handles, 'units','normalized');
  else
...
if (display)
  ...
  rectangle ('Position', [0 0 1 1])
    ...
    if (exist ('OCTAVE_VERSION', 'builtin'))
      set (text_handles, 'units', 'normalized');
    else
...
if (first_redraw || ~isfield (obj.redraw_cache, 'max_legend_ind'))
  legend_text_pos = get (obj.legend_text_handles, 'Extent');
                                % On first redraw we get all extents
else
...
if (~isempty (legend_text_pos))
  ...
  if (first_redraw)
    % Cache the index of which text object is the rightmost
    obj.redraw_cache.max_legend_ind = max_ind; 
  end
...
for a=(1:numel (obj.facet_axes_handles))
  ...
  set (obj.facet_axes_handles(a), 'Position',...
       facet_pos{a}+[obj.multi.orig(2)+spacing_w-min_facet_x obj.multi.orig(1)+spacing_h-min_facet_y 0 0]);
                                %was spacing_x-min_facet_y
end
...
if (first_redraw || ~isfield (obj.redraw_cache, 'max_facet_ind'))
  facet_text_pos = get (obj.facet_text_handles, 'Extent');
                                % On first call get all positions
else
...
if (~isempty (facet_text_pos))
  ...
  axe_text_pos = cellfun (@(a) get(a, 'Position'), temp_handles, ...
                          'UniformOutput', false);
...
if (~isempty (legend_text_pos))
                         % Place relative to legend axis if we have one
...
if (obj.force_ticks || (obj.wrap_ncols > 0))
                       % && ~isempty(strfind(obj.facet_scale,'free')))
  ...
  for r=(1:nr)
    for c=(1:nc)
      if (ind <= length(obj.facet_axes_handles))
        ...
        set (obj.facet_axes_handles(ind), 'Position',...
             [min_facet_x+(neww+spacing_w)*(c-1) min_facet_y+(newh+spacing_h)*(nr-r) neww newh]);
      end
      ind = ind+1;
    end
  end
    ...
    if ((strcmp(obj.facet_space,'free') || strcmp(obj.facet_space,'free_x')) ...
        && ~strcmp(obj.facet_scale,'independent'))
    ...
    if ((strcmp (obj.facet_space, 'free') || strcmp (obj.facet_space, 'free_y'))...
        && ~strcmp (obj.facet_scale,'independent'))
      ydataw = zeros (1,nr);
      for r =(1:nr)
    ...
    for r =(1:nr)
        for c=(1:nc)
          ...
          if (r == nr)
            ...
            % Place facet
          set (obj.facet_axes_handles(r, c), 'Position',...
               [x_offset y_offset facet_width(c) facet_height(r)]);
        end
    end
end
