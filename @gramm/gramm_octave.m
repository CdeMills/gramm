    ...
    %     used for every row of y.
      ...
      if (nargin > 0)  
        if (exist ('OCTAVE_VERSION', 'builtin'))
          ...
          addParamValue (p, 'x', []);
          addParamValue (p, 'y', []);
          addParamValue (p, 'ymin', []);
          addParamValue (p, 'ymax', []);
          addParamValue (p, 'z', []);
          
          %# Other aesthetics are string-value pairs
          addParamValue (p, 'color', []);
          addParamValue (p, 'group', []);
          addParamValue (p, 'lightness', []);
          addParamValue (p, 'linestyle', []);
          addParamValue (p, 'size', []);
          addParamValue (p, 'marker', []);
          addParamValue (p, 'subset', []);
          addParamValue (p, 'row', []);
          addParamValue (p, 'column', []);
          ...
          %# in 2D such as X and Y
            if (isrow (p.Results.(p.Parameters{pr})))
              obj.aes.(p.Parameters{pr}) = shiftdim (p.Results.(p.Parameters{pr}));
            else
              obj.aes.(p.Parameters{pr}) = p.Results.(p.Parameters{pr});
            end
          end
