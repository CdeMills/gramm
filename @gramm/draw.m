...
if (obj.updater.first_draw || 1 == obj.updater.facet_updated)
...
% Get colormap
cmap = get_colormap (length (uni_color), length (uni_lightness),...
                     obj.color_options);

% Initialize results structure (n_groups is an overestimate if
% there are redundant groups
n_groups = prod ([length(uni_row) length(uni_column) length(uni_marker) ...
                  length(uni_size) length(uni_linestyle) length(uni_color) ...
                  length(unique(temp_aes.group))]);
...
if (isempty(draw_data.dodge_avl_w) || 0 == draw_data.dodge_avl_w)
...
for ind_row = (1:length (uni_row))
    ...
    for ind_column = (1:length (uni_column))
      ...
      if (obj.updater.first_draw) % If we are in the normal case (first draw() call)
        ...
        if (1 == obj.updater.facet_updated) % If faceting was updated from one to multiple facets
          if (1 == ind_column && 1 == ind_row) % If we are in the first facet
   ...
   % Store content of first facet for copying it on the other facets !
        ...
        if (-1 == obj.updater.facet_updated) % If facets were updated from many to one facets
          if (1 == ind_column && 1 == ind_row)
      ...
      [dodge_data.fallback, dodge_data.x, dodge_data.color, ...
       dodge_data.lightness, dodge_data.ind, dodge_data.n]...
      = dodge_comp (temp_aes.x(sel_column), temp_aes.color(sel_column),...
                    temp_aes.lightness(sel_column), uni_color, uni_lightness);
   ...
   % We create the groups only within lightness and subplots for speed
                ...
                for ind_group = (1:length (uni_group))
                  ...
                  if (~isempty (sel))
                                ...
                                % Fill up the draw_data structure
                                % passed to the individual geoms
      ...
      if (obj.updater.first_draw || obj.updater.facet_updated)
        if (length (uni_column) > 1)
          ...
          if (1 == ind_row)
            ...
            = [obj.facet_text_handles ...
                  text('Interpreter', 'none', 'String', column_string, ...
                       'Rotation', 0, ...
                       'Units', 'normalized', ...
                       'Position', [0.5 1.05 2], ...
        ...
        if (length (uni_row) > 1)
          ...
          if (ind_column == length (uni_column))
            obj.facet_text_handles...
            =[obj.facet_text_handles ...
                 text('Interpreter', 'none', 'String', row_string, ...
                      'Rotation', -90, ...
                      'Units', 'normalized', ...
                      'Position', [1.05 0.5 2], ...
                      'BackgroundColor', 'none', ...
...
if (obj.with_legend)
  ...
  if (length (uni_color) > 1)
    ...
    = [obj.legend_text_handles...
          text(1, obj.legend_y, obj.aes_names.color, 'FontWeight','bold',...
               'Interpreter', 'none', 'fontSize', 12, ...
               'Parent', obj.legend_axe_handle)];
    obj.legend_y = obj.legend_y-legend_y_step;
    for ind_color =(1:length (uni_color))
      plot ([1 2], [obj.legend_y obj.legend_y], '-', ...
            'Color', color_legend_map(ind_color,:), ...
            'lineWidth', 3, 'Parent', obj.legend_axe_handle)
...
% rectangle ('Position', [1.25 obj.legend_y-0.25 0.5 0.5], 'EdgeColor', 'none', 'FaceColor', color_legend_map(ind_color,:));
      ...
      = [obj.legend_text_handles...
            text(2.5, obj.legend_y, num2str (uni_color{ind_color}),...
                 'Interpreter', 'none', 'Parent', obj.legend_axe_handle)];
      obj.legend_y = obj.legend_y-legend_y_step;
    end
  end
  ...
  if (length (uni_lightness) > 1)
    ...
    = [obj.legend_text_handles...
          text(1, obj.legend_y, obj.aes_names.lightness, ...
               ...
               'fontSize', 12, 'Parent', obj.legend_axe_handle)];
    obj.legend_y = obj.legend_y-legend_y_step;
    for ind_lightness = (1:length (uni_lightness))
      plot ([1 2], [obj.legend_y obj.legend_y], '-', ...
            'Color', lightness_legend_map(ind_lightness,:), ...
            'lineWidth', 3, 'Parent', obj.legend_axe_handle)
...
% image(ones(1, length(obj.continuous_color_colormap))+0.5, ...
    ...
    tmp_N = 100;
    imagesc ([1 1.5], [obj.legend_y-legend_y_step*2 obj.legend_y], ...
             linspace (min (min (obj.plot_lim.minc)), ...
                       max (max (obj.plot_lim.maxc)), tmp_N)',...
             'Parent', obj.legend_axe_handle);
    line ([1.8 2.2 ; 1.8 2.2 ;1.8  2.2]', ...
          [obj.legend_y obj.legend_y;
           obj.legend_y-legend_y_step obj.legend_y-legend_y_step
           obj.legend_y-legend_y_step*2 obj.legend_y-legend_y_step*2 ]', ...
          'Color', 'k', 'Parent', obj.legend_axe_handle)
    ...
    obj.legend_text_handles...
    = [ obj.legend_text_handles...
           text(2.5, obj.legend_y-legend_y_step, ...
                num2str ((max (max (obj.plot_lim.maxc)) + min (min (obj.plot_lim.minc)))/2), ...
                'Parent', obj.legend_axe_handle)];
    
    obj.legend_text_handles ...
    = [obj.legend_text_handles ...
          text(2.5, obj.legend_y-legend_y_step*2, ...
               num2str (min (min (obj.plot_lim.minc))), ...
               'Parent', obj.legend_axe_handle)];
  ...
  if (length (uni_marker) > 1)
    ...
    obj.legend_y = obj.legend_y-legend_y_step;
    for ind_marker=(1:length (uni_marker))
      plot(1.5, obj.legend_y, markers{ind_marker},...
  ...
  if (length (uni_linestyle) > 1)
    ...
    obj.legend_text_handles...
    =[obj.legend_text_handles...
         text(1, obj.legend_y, obj.aes_names.linestyle, 'FontWeight', 'bold', ...
              'Interpreter', 'none', 'fontSize', 12, ...
              'Parent', obj.legend_axe_handle)];
    obj.legend_y = obj.legend_y-legend_y_step;
    for ind_linestyle= (1:length (uni_linestyle))
      plot([1 2], [obj.legend_y obj.legend_y], line_styles{ind_linestyle}, ...
  ...
  if (length (uni_size) > 1)
    ...
    obj.legend_text_handles...
    = [obj.legend_text_handles...
          text(1, obj.legend_y, obj.aes_names.size, 'FontWeight', 'bold', ...
               'Interpreter', 'none', 'fontSize', 12, ...
               'Parent', obj.legend_axe_handle)];
    obj.legend_y = obj.legend_y-legend_y_step;
    for ind_size = (1:length (uni_size))
      plot([1 2], [obj.legend_y obj.legend_y], ...
           'lineWidth', sizes(ind_size)/4, 'Color', [0 0 0],...
           'Parent', obj.legend_axe_handle)
      plot(1.5, obj.legend_y, 'o', 'markerSize', sizes(ind_size), ...
           'MarkerEdgeColor', 'none', 'MarkerFaceColor', [0 0 0], ...
           'Parent', obj.legend_axe_handle)
      obj.legend_text_handles...
      = [obj.legend_text_handles...
...
if (-1 == obj.updater.facet_updated)
...
for ind_row = (1:length (uni_row)) % Loop over rows
  for ind_column = (1:length (uni_column)) %Loop over columns
    ...
    if (obj.continuous_color)
      ...
      set (ca, 'CLimMode', 'manual', 'CLim',...
           [min(min(obj.plot_lim.minc)) max(max(obj.plot_lim.maxc))]);
    end
        
                             %Ad hoc limit correction for empty facets
    obj.plot_lim.maxy(obj.plot_lim.miny == obj.plot_lim.maxy) ...
    = obj.plot_lim.maxy(obj.plot_lim.miny == obj.plot_lim.maxy) + 0.01;
    obj.plot_lim.maxx(obj.plot_lim.minx == obj.plot_lim.maxx) ...
    = obj.plot_lim.maxx(obj.plot_lim.minx == obj.plot_lim.maxx) + 0.01;
    obj.plot_lim.maxz(obj.plot_lim.minz == obj.plot_lim.maxz)...
    = obj.plot_lim.maxz(obj.plot_lim.minz == obj.plot_lim.maxz) + 0.01;
  ...
  %# with length > 1
        ...
        switch obj.facet_scale
          case 'fixed'
            ...
            temp_zscale = 'global';
                      ...
                      % Both XLim and YLim are linked across all plots
      ...
      if (~isempty (temp_aes.z)) % Only do the Z limit stuff if we have z data
        ...
        has_xtick = obj.force_ticks || ... %Plot has xticks if forced
                    (-1 == obj.wrap_ncols && length (uni_row)) == ind_row || ... %Or if we're in facet grid mode and we are in the last row
        ...
        has_ytick = obj.force_ticks || ... %Plot has xticks if forced
                    (-1 == obj.wrap_ncols && 1 == ind_column) || ... %Or if we're in facet grid mode and are in the first column
      ...
      if (obj.x_factor)
        ...
        if (has_xtick)
          ...
          if (isprop (ca, 'TickLabelInterpreter'))
                             % Just try it (doesn't exist in pre-2014b)
      ...
      if (1 == ind_column || ~isempty (temp_aes.z) ...
          || (obj.wrap_ncols > 0 && mod (ind_column, obj.wrap_ncols) == 1)) 
        ylabel (ca, obj.aes_names.y, 'Interpreter','none');
                        % ,'Units', 'normalized', 'position', [-0.2 0.5 1]
      end
      if ((length(uni_row) == ind_row && obj.wrap_ncols <=0 ) ...
          || (obj.wrap_ncols>0 && (length (uni_column) - ind_column) < obj.wrap_ncols)...
          || ~isempty (temp_aes.z))
        xlabel (ca, obj.aes_names.x, 'Interpreter','none')
      end
                                % If we have z data
      if (~isempty (temp_aes.z))
        zlabel (ca, obj.aes_names.z, 'Interpreter', 'none') % As z label
    ...
    if (obj.abline.on)
      ...
      for line_ind = (1:length (obj.abline.intercept))
        if (~isnan (obj.abline.intercept(line_ind)))
                                %abline
          plot (xl, xl*obj.abline.slope(line_ind)+obj.abline.intercept(line_ind), ...
                obj.abline.style{line_ind}, 'Parent', ca);
        else
          if (~isnan (obj.abline.xintercept(line_ind)))
                                %vline
            yl = get (ca, 'ylim');
            plot ([obj.abline.xintercept(line_ind) obj.abline.xintercept(line_ind)],...
                  yl, obj.abline.style{line_ind}, 'Parent', ca);
          else
            if (~isnan (obj.abline.yintercept(line_ind)))
                                %hline
              plot (xl, [obj.abline.yintercept(line_ind) obj.abline.yintercept(line_ind)], ...
                    obj.abline.style{line_ind}, 'Parent', ca);
            else
              temp_x = linspace(xl(1), xl(2), 100);
              plot (temp_x, obj.abline.fun{line_ind}(temp_x), ...
                    obj.abline.style{line_ind}, 'Parent', ca);
            end
          end
        end
...
%painters renderer
if (exist ('OCTAVE_VERSION', 'builtin'))
  %# no specific issue about renderer ?
else
  if (verLessThan ('matlab', '8.4'))
...
% Return maximum of an array or of a cell of arrays
  if (iscell (in))
    out = max (cellfun (@max, in(~cellfun (@isempty, in))));
                                %Cellfun on non empty cells
  else
    out = max (in);
  end
end
...
% Return minimum of an array or of a cell of arrays
  if (iscell (in))
    out = min (cellfun (@min, in(~cellfun (@isempty, in))));
                                %Cellfun on non empty cells
  else
    out = min (in);
  end
