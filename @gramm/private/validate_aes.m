...
for k = (1:length (fields))
  if (numel (out.(fields{k})) > 0) %Ignore empty ones
    if (aes_length ==-1 && numel (out.(fields{k}))~= 1)
      ...
      if (aes_length ~= numel (out.(fields{k})) && numel (out.(fields{k}))~=1)
                                %# Handle special case of size
...
for k=(1:length (fields))
  if (isempty (aes.(fields{k})) && ...
      sum (strcmp (fields{k},{'z' 'ymin' 'ymax'}))==0)
                       %If z, ymin, ymax are empty we leave them empty
