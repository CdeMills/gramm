...
%# cellstrs, and sorts according to sortopts
  ...
  if (numel (sortopts) > 1) % Custom ordering
    ...
    if (length (sortopts) == length (y)) %# Correct length ?
      if (isnumeric (sortopts) && sum (sort (sortopts) == (1:length (y))') ==numel (y))
%# If we have integers and all numbers from 1 to N are there we probably have indices
    ...
    switch (sortopts)
      ...
      case -1
        y = flipud (sort (y));
%# We use flipud instead of the 'descend' option because somehow it
%# isn't supported for cellstr.
    end
  end
