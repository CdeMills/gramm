...
function hnew = octave_copyobj (horig, hparent = 0)
  ...
  hnew = zeros (size (horig));
  if (isscalar (hparent))
    hparent = repmat (hparent, size (horig));
  end

  for indi = (length (horig(:)):-1:1)
    ## compatibility of input handles
    kididx = find (strcmp (alltypes, get (horig(indi)).type));
    paridx = find (strcmp (alltypes, get (hparent(indi)).type));

    if (kididx <= paridx)
      error ("copyobj: %s object can't be children to %s.",
             alltypes{kididx}, alltypes{paridx});
    elseif (1 == nargin)
      str = hdl2struct (horig(indi));
      hnew(indi) = struct2hdl (str);
    else
      str = hdl2struct (horig(indi));
      hnew(indi) = struct2hdl (str, hparent(indi));
    endif
  end
