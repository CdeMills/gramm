...
if (isempty (params.dodge))
  if (sum (strcmp (params.geom,'bar')) > 0 && draw_data.n_colors > 1)
                                %If we have a bar as geom, we dodge
    params.dodge = 0.6;
  else
    params.dodge = 0;
  end
...
if (iscell (draw_data.x) || iscell (draw_data.y))
    ...
    if (params.interp_in > 0)
      ...
      [x,y] = cellfun(@(x,y) deal(uni_x, interp1(x, y, uni_x, 'linear')),...
                      draw_data.x, draw_data.y, 'UniformOutput', false);
      y = padded_cell2mat (y);
    else
      ...
      x_diff = max (x)-min (x);
      if (any (x_diff(1:end-1) >diff(uni_x)/10))
                               % More than a tenth of delta x variation
  ...
  if (params.bin_in > 0)
    ...
    uni_x = bincenters;
    sel = (binind ~=0);
% histcounts can return zero as bin index if NaN data we remove them here
...
% the original data ?
if (params.setylim)
  if (sum(sum(isnan(yci))) ~= numel(yci))
                                %We only do this if yci is not weird
    if (obj.firstrun(obj.current_row, obj.current_column))
                          %Initialize for the first run in the subplot
...
if (sum (strcmp (params.geom, 'bar')) > 0)
  if (obj.plot_lim.miny(obj.current_row, obj.current_column) > 0)
                                % Values above zero -> change miny
    obj.plot_lim.miny(obj.current_row, obj.current_column) = 0;
  end
  if (obj.plot_lim.maxy(obj.current_row, obj.current_column) < 0)
                                %Values below zero -> change maxy
...
if (~strcmp(params.interp, 'none'))
  ...
  if (obj.polar.is_polar && obj.polar.is_polar_closed && ~strcmp (params.interp,'polar'))
    ...
    params.interp = 'polar';
    %% If the plot is polar we override the interpolation type do an optimal fft interpolation
  end
...
hndl = plotci (obj, uni_x, ymean, yci, draw_data, ...
               params.geom, params.dodge, params.width);
